...
// `office_print::convert_with_options` reads from the filesystem and is not
...
fn main() {
    ...
    use office_print::config::{ConvertOptions, PaperSize, SlideRange};
    ...
    match office_print::convert_with_options(input, &options) {
        Ok(result) => {
            let pdf = result.as_pdf_bytes().unwrap();
            fs::write(output, pdf).expect("failed to write PDF");
            println!("Wrote {} bytes to {output}", pdf.len());
        }
        Err(e) => {
