...
//! - `office_print_VALIDATE_PDF=1` environment variable is set
...
fn validate_docx_table_pdf() {
    ...
    let result = office_print::convert(&path).expect("DOCX conversion should succeed");
    assert!(
...
fn validate_pptx_sample_pdf() {
    ...
    let result = office_print::convert(&path).expect("PPTX conversion should succeed");
    assert!(
        result.as_pdf_bytes().unwrap().starts_with(b"%PDF"),
        "output should be a valid PDF"
    );
...
fn validate_xlsx_temperature_pdf() {
    ...
    let result = office_print::convert(&path).expect("XLSX conversion should succeed");
    assert!(
        result.as_pdf_bytes().unwrap().starts_with(b"%PDF"),
        "output should be a valid PDF"
    );
