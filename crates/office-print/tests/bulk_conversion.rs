...
//!
//! Run with: `cargo test -p office_print --test bulk_conversion -- --nocapture --ignored`
...
// See: https://github.com/developer0hye/office_print/issues/77
// ---------------------------------------------------------------------------
...
// See: https://github.com/developer0hye/office_print/issues/82
// ---------------------------------------------------------------------------
...
fn convert_file(path: &Path, format: Format) -> FileResult {
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        office_print::convert_bytes(&data, format, &ConvertOptions::default())
    }));

    match result {
        Ok(Ok(convert_result)) => {
            let pdf_size = convert_result.as_pdf_bytes().unwrap().len();
            FileResult {
                path: path.to_path_buf(),
