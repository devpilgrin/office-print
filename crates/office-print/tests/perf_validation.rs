...
fn ensure_font_cache_warm() {
    WARM_UP.call_once(|| {
        ...
        let _ = office_print::convert_bytes(&data, Format::Docx, &ConvertOptions::default());
    });
}
...
fn timed_convert(data: &[u8], format: Format, label: &str) -> Duration {
    ...
    let result = office_print::convert_bytes(data, format, &opts).unwrap();
...
fn perf_font_cache_second_conversion_faster() {
    ...
    let result1 = office_print::convert_bytes(&data, Format::Docx, &opts).unwrap();
    let m1 = result1
        .metrics
        .as_ref()
    ...
    let result2 = office_print::convert_bytes(&data, Format::Docx, &opts).unwrap();
    let m2 = result2
        .metrics
        .as_ref()
        .expect("metrics should be populated");
...
fn perf_cross_format_cached_conversion() {
    ...
    let _ = office_print::convert_bytes(&docx_data, Format::Docx, &opts).unwrap();
...
fn tier_builders_produce_valid_documents() {
    ...
    let r = office_print::convert_bytes(&medium_docx, Format::Docx, &opts).unwrap();
    assert!(
    ...
    let r = office_print::convert_bytes(&medium_pptx, Format::Pptx, &opts).unwrap();
    assert!(
        !r.as_pdf_bytes().unwrap().is_empty(),
    ...
    let r = office_print::convert_bytes(&medium_xlsx, Format::Xlsx, &opts).unwrap();
    assert!(
        !r.as_pdf_bytes().unwrap().is_empty(),
