...
//!
//! Run with:
//!   cargo test -p office_print --test artifact_generator -- --ignored --nocapture
//!
...
fn test_visual_comparison_all() {
    ...
    for (idx, pair) in pairs.iter().enumerate() {
        ...
        let convert_result =
            office_print::convert_bytes(&input_bytes, format, &ConvertOptions::default());

        let pdf_bytes = match convert_result {
            Ok(result) => result.as_pdf_bytes().unwrap().to_vec(),
            Err(e) => {
                println!("FAIL ({e})");
                results.push(FileResult {
                    input: pair.input.clone(),
