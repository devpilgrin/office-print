...
/// Compile Typst markup to a PagedDocument. Native targets.
#[cfg(not(target_arch = "wasm32"))]
pub fn compile_to_document(
    typst_source: &str,
    images: &[ImageAsset],
    font_paths: &[PathBuf],
) -> Result<PagedDocument, ConvertError> {
    let world = MinimalWorld::new(typst_source, images, font_paths);
    compile_document_inner(&world)
}

/// Compile Typst markup to a PagedDocument. WASM targets.
#[cfg(target_arch = "wasm32")]
pub fn compile_to_document(
    typst_source: &str,
    images: &[ImageAsset],
...
/// Shared compilation: world → PagedDocument.
fn compile_document_inner(world: &MinimalWorld) -> Result<PagedDocument, ConvertError> {
...
/// Export a PagedDocument to PDF bytes (used by both compile_to_pdf and pipeline).
pub fn export_pdf(
...
fn compile_to_pdf_inner(
    ...
    let document = compile_document_inner(world)?;
    export_pdf(&document, pdf_standard, tagged, pdf_ua)
}
