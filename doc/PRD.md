# office-print - Product Requirements Document (PRD)

## 1. Overview

**office-print** is a library and CLI tool that converts DOCX, XLSX, and PPTX files to PDF using pure Rust.
...
| PNG output | Single page → PNG bytes, multi-page → one PNG per page |
| JPEG output | Single page → JPEG bytes (configurable quality, default 92) |
...
```rust
use office-print::{Document, ConvertOptions, Format};

// Simple usage
let pdf_bytes = office-print::convert("input.docx")?;
...
let pdf_bytes = office-print::convert_with_options("input.xlsx", &options)?;

// Convert from bytes
let docx_bytes = std::fs::read("input.docx")?;
let pdf_bytes = office-print::convert_bytes(&docx_bytes, Format::Docx, &options)?;
...
```bash
# Basic conversion
office-print input.docx                     # → input.pdf
office-print input.pptx -o output.pdf       # specify output path

# Options
office-print input.xlsx --sheets "Sheet1,Sheet2"   # specific sheets only
office-print input.pptx --slides 1-5               # specific slides only
office-print input.docx --paper a4 --landscape     # paper settings
office-print input.docx --font-path ./fonts        # font path

# Batch conversion
office-print *.docx --outdir ./pdfs/               # batch convert multiple files

# Info
office-print --version
office-print --help
```
...
| **Manual comparison** | MS Office PDF output vs office-print output comparison |
